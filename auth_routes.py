...
from fastapi.encoders import jsonable_encoder # Pour encoder les données en JSON
...
async def connexion(utilisateur: ConnexionModel, Authorize: AuthJWT = Depends()):
    ...
    if db_utilisateur and check_password_hash(getattr(db_utilisateur, "password"), utilisateur.password): # utiliser getattr pour accéder à l'attribut password de l'objet db_utilisateur (getattr(db_utilisateur, "password")), au lieur de db_utilisateur.password
        ...
        return jsonable_encoder(reponse)  # Utilisation de jsonable_encoder pour encoder la réponse en JSON
